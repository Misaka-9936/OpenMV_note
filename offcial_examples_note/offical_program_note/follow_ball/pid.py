...
class PID:
    ...
    def get_pid(self, error, scaler):
        ...
        if abs(self._kd) > 0 and dt > 0:
            ...
            output = output + self._kd*derivative
        output = output * scaler
        if abs(self._ki) > 0 and dt > 0:
            self._integrator = self._integrator + (error*self._ki)*scaler*delta_time
            if self._integrator < -self._imax: 
                self._integrator = -self._imax
            elif self._integrator > self._imax:
                self._integrator = self._imax
            output = output + self._integrator
        return output
